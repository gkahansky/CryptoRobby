...
namespace Crypto.RuleEngine.Patterns
{
    public class SpringPattern : Pattern
    {
        ...
        public override bool CheckPattern(Kline kline)
        {
            ...
            _logger.Log(String.Format("{6} {7}: Low: {0}, High: {1}, Spring: {2}, Last Price: {3}, Highest Price: {4}, time: {5}", Low, High, Spring, LastPrice, HighPrice, TickTime, this.Symbol, this.Interval));
            LastPrice = avgPrice;
            return false;
        }
        ...
        private decimal CheckLow(decimal avgPrice, decimal low)
        {
            ...
            {
                low = avgPrice;
                _logger.Log(string.Format("LOW Price update {3} {4}: {0}, Top Range: {1}, Bottom Range: {2}", low, lowTop, lowBottom, this.Symbol, this.Interval));
            ...
            else if (avgPrice > lowTop)
            {
                ...
                Rules["Low"] = true;
                High = avgPrice;
                _logger.Log(String.Format("LOW RULE ACHIEVED {0} {1}!!! Moving to HIGH Rule", this.Symbol, this.Interval));
            }
            return low;
        }
        ...
        private decimal CheckHigh(decimal avgPrice, decimal high, decimal spring)
        {
            ...
            {
                high = avgPrice;
                _logger.Log(string.Format("HIGH Price update {3} {4}: {0}, Top Range: {1}, Bottom Range: {2}", high, highTop, highBottom, this.Symbol, this.Interval));
                if (avgPrice > highTop)
                    Trend = true;
            }
            else if (avgPrice < highBottom)
            {
                ...
                Spring = High;
                Rules["High"] = true;
                _logger.Log(String.Format("HIGH RULE ACHIEVED {0} {1}!!! Moving to SPRING Rule", this.Symbol, this.Interval));
            }
            return high;
        ...
        private decimal CheckSpring(decimal avgPrice, decimal high)
        {
            ...
            if (avgPrice > springTop)
            {
                ...
                Trend = true;
                Pivot = avgPrice;
                _logger.Log(String.Format("SPRING RULE ACHIEVED {0} {1}!!! Moving to COMPLETION Rule", this.Symbol, this.Interval));
            }
            else if (avgPrice < Spring)
            {
                ...
                {
                    Spring = avgPrice;
                    _logger.Log(string.Format("SPRING Price update {4} {5}: {0}, Top Range: {1}, Bottom Range: {2}, Limit for pattern break: {3}", Spring, springTop, springBottom, springFloor, this.Symbol, this.Interval));
                }
        ...
        private decimal CheckComplete(decimal avgPrice, decimal spring, decimal high, decimal pivot)
        {
            ...
            {
                Rules["Complete"] = true;
                _logger.Log(String.Format("SPRING PATTERN DETECTED - BUY {0} NOW! {1}", this.Symbol, TickTime));
            }
